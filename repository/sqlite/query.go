...
func FindPlanQuery(planID string) bob.Query {
	return sqlite.Select(
		...
		sm.Where(sqlite.Quote("stripe_price_id").EQ(sqlite.Arg(planID))),
	)
}
...
func GetPlansQuery() bob.Query {
	return sqlite.Select(
		sm.From("stripeflow_plans"),
		sm.Where(sqlite.Quote("is_active").EQ(sqlite.Arg(true))),
...
func FindSubscriptionByStripeIDQuery(stripeSubID, stripeCustomerID string) bob.Query {
	return sqlite.Select(
		sm.From("stripeflow_subscriptions"),
		sm.Where(sqlite.Quote("stripe_subscription_id").EQ(sqlite.Arg(stripeSubID)).And(sqlite.Quote("stripe_customer_id").EQ(sqlite.Arg(stripeCustomerID)))),
	)
}
...
func DeleteSubscriptionQuery(id string) bob.Query {
	return sqlite.Delete(
		...
		dm.Where(sqlite.Quote("id").EQ(sqlite.Arg(id))),
	)
}
